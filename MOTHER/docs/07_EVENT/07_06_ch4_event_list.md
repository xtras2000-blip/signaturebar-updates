...
| `ch4_ev_003` | 아이의 비밀 | major | conditional · has `mem_ch3_doyun_fight`† AND time_block=afternoon | guilt | `mem_ch4_child_secret` | CH5 (마지막 등원 — 신발장 앞 같은 침묵의 대비) |
| `ch4_ev_004` | 학원 보낼까 고민 | minor | conditional · has `mem_ch3_compare_anxiety`† AND time_block=night | guilt | `mem_ch4_academy_debate` | CH5 (입학 준비물 구매 — "필요한 것"의 기준 재등장) |
...
| `ch4_ev_013` | 시어머니의 인정 | minor | conditional · has `mem_ch2_holiday_boundary`† AND holiday=chuseok | reconciliation | `mem_ch4_inlaw_acknowledge` | CH5 (입학식 — 가족석 구도, 06_03 아크 완결) |
...
- † 표시 태그는 07_04(CH2)·07_05(CH3) 카탈로그 미작성 상태의 합리적 가정치다. 07_04/07_05 작성 시 해당 카탈로그의 실제 `memory_tag`와 반드시 동기화(교차 검증)한다.
...
| `ch4_ev_003` 아이의 비밀 | `mem_ch3_doyun_fight`† | CH3 친구 갈등 | 도윤과의 다툼 전력이 "말 안 하는 이유"가 된다 |
| `ch4_ev_004` 학원 고민 | `mem_ch3_compare_anxiety`† | CH3 비교 불안 | 맘카페 비교 불안이 사교육 고민으로 재점화 |
...
| `ch4_ev_013` 시어머니의 인정 | `mem_ch2_holiday_boundary`† | CH2 명절 선 긋기 | "네 방식대로 잘 키웠다" 또는 침묵 (06_03 아크) |
...
- † 가정 태그(`mem_ch2_holiday_boundary`, `mem_ch3_doyun_fight`, `mem_ch3_compare_anxiety`)는 07_04/07_05 확정 전까지 빌드 린트에서 "참조 선행" 경고로 표기한다.
